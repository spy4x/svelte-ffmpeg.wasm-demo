...
export const movies = {
  ...
  update: async (movie: MovieVM): Promise<void> => {
    ...
    // Detect files to be uploaded (clips)
    const clipsChangedIds = movie.clips.filter(c => c.file).map(a => a.id);

    if (clipsChangedIds.length) {
      const [error, result] = await request<{
        clips: { id: string; path: string; url: string }[];
      ...
      const uploadPromises =
        result.clips?.map(a => {
          ...
          if (clip?.file) {
            const file = new File([clip.file], clip.id);
            return uploadFile(a.url, file);
          }
        }) || [];
